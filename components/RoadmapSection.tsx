    ...
    <div className="mt-4 border border-border rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-text-primary text-sm">達成ロードマップ</span>
        </div>
        <svg
      ...
      {isOpen && (
        <div className="px-4 pb-4 pt-2 animate-fade-slide">
          {/* ゴール */}
          <div className="mb-4">
            <p className="text-sm font-semibold text-text-primary mb-1">🎯 目標</p>
            <p className="text-sm ">{roadmap.goal}</p>
          </div>

          {/* 前提スキル */}
          <div className="mb-4">
            <p className="text-sm font-semibold text-text-primary mb-2">📋 前提スキル</p>
              ...
              {roadmap.prerequisites.map((prereq, i) => (
                <div key={i} className="flex items-start gap-2 bg-background rounded-lg p-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs ">{prereq.description}</p>
                    <p className="text-xs  mt-1">目標: {prereq.target}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* 段階的練習 */}
          <div className="mb-4">
            <p className="text-sm font-semibold text-text-primary mb-2">🏋️ 段階的練習メニュー</p>
            <div className="relative pl-4 border-l-2 border-border space-y-4">
              {roadmap.drills.map((drill, i) => (
                <div key={i} className="relative">
                  ...
                  <p className="text-xs font-bold  uppercase tracking-wide">{drill.phase}</p>
                  <p className="text-sm font-medium text-text-primary">{drill.name}</p>
                  <p className="text-xs ">{drill.detail}</p>
                  <p className="text-xs  mt-1">推奨: {drill.reps}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-background rounded-lg p-3 flex items-center gap-2">
            <span className="text-lg">⏱️</span>
            <div>
              <p className="text-xs ">達成目安期間</p>
              <p className="text-sm font-semibold text-text-primary">{roadmap.timeEstimate}</p>
            </div>
          </div>
        </div>
      )}
